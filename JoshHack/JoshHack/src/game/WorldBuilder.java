...
public class WorldBuilder {
	...
	public void innerloop2(int x) {
		for (int y = 0; y < height; y++) {
			innerloop1(x, y);
		}
	}

	public void innerloop1(int x, int y) {
	...
	public void bigloop(Tile[][][] tiles2, int x, int y) {
		for (int z = 0; z < depth; z++) {
			int floors = 0;
			int rocks = 0;
	...
	public void bigloop2(int z, int x) {
		for (int y = 0; y < height; y++){
	...
	private void removeRegion(int region, int z){
		for (int x = 0; x < width; x++){
			smallloop3(region, z, x);
		}
	}

	public void smallloop3(int region, int z, int x) {
		for (int y = 0; y < height; y++){
	...
	private int fillRegion(int region, int x, int y, int z) {
		...
		while (!open.isEmpty()){
			...
			size = bigloop3(region, size, open, p);
		}
		return size;
	}

	public int bigloop3(int region, int size, ArrayList<Point> open, Point p) {
	...
	private void connectRegionsDown(int z){
		...
		for (int x = 0; x < width; x++){
			bigloop4(z, connected, x);
		}
	}

	public void bigloop4(int z, List<Integer> connected, int x) {
		for (int y = 0; y < height; y++){
			int r = regions[x][y][z] * 1000 + regions[x][y][z+1];
			nestedif(z, connected, x, y, r);
		}
	}

	public void nestedif(int z, List<Integer> connected, int x, int y, int r) {
	...
	private void connectRegionsDown(int z, int r1, int r2){
		...
		dowhileloop(z, candidates);
	}

	public void dowhileloop(int z, List<Point> candidates) {
		int stairs = 0;
		do{
	...
	public List<Point> findRegionOverlaps(int z, int r1, int r2) {
		...
		for (int x = 0; x < width; x++){
			foriflong(z, r1, r2, candidates, x);
		}
	...
	public void foriflong(int z, int r1, int r2, ArrayList<Point> candidates, int x) {
		for (int y = 0; y < height; y++){
			if (tiles[x][y][z] == Tile.FLOOR
					&& tiles[x][y][z+1] == Tile.FLOOR
					&& regions[x][y][z] == r1 
